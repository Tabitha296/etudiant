...
$motcle = $_GET['rechercher'] ?? '';
if ($motcle != '') {
    $motcle = str_replace(['%', '_'], ['\%', '\_'], $motcle);
    $reponse = "SELECT * FROM etudiant WHERE nom LIKE :mc";
    $statment = $pdo->prepare($reponse);
    $statment->execute([':mc' => "%$motcle%"]);
    $etudiants = $statment->fetchAll(PDO::FETCH_ASSOC);

    $message = (empty($etudiants)) ? "Il n'y a pas de résultat pour \"$motcle\"." : null;
} else {
